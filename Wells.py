...
def tab_data():
    ...
    with colA:
        st.subheader("Training Excel (.xlsx)")
        xlsx_file = st.file_uploader("Upload training Excel", type=["xlsx"], key="xlsx_up")
    ...
    with colB:
        st.subheader("Live CSV (.csv)")
        csv_file = st.file_uploader("Upload live CSV", type=["csv"], key="csv_up")
...
def main():
    ...
    st.caption("Upload training XLSX & live CSV, pick features, train, and detect anomalies with a configurable threshold.")
